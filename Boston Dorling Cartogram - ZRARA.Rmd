---
title: "Jackson, MS Metro Cartogram"
author: "Jason Sills"
date: "12/5/2020"
...
```{r}
grep( "^JACK", crosswalk$msaname, value=TRUE ) 
```


```{r}
these.jax <- crosswalk$msaname == "JACKSON, MS"
these.fips <- crosswalk$fipscounty[ these.jax ]
...
get_acs( geography = "tract", variables = "B01003_001",
         state = "28", county = county.fips[state.fips=="28"], geometry = TRUE ) %>% 
...
jackson <- jackson[ ! st_is_empty( jackson ) , ]

# convert sf map object to an sp version
jackson.sp <- as_Spatial( jackson )
...
jackson.sp <- spTransform( jackson.sp, CRS("+init=epsg:3395"))
jackson.sp <- jackson.sp[ jackson.sp$POP != 0 & (! is.na( jackson.sp$POP )) , ]

# convert census tract polygons to dorling cartogram
# no idea why k=0.03 works, but it does - default is k=5
jackson.sp$pop.w <- jackson.sp$POP / 9000 # max(msp.sp$POP)   # standardizes it to max of 1.5
jackson_dorling <- cartogram_dorling( x=jackson.sp, weight="pop.w", k=0.05 )
plot( jackson_dorling )
```


```{r}
class(jackson_dorling)
```
...
tm_shape( jackson_dorling, bbox=bb ) + 
  tm_polygons( col="POP", n=10, style="quantile", palette="Spectral" ) +
  tm_layout( "Greater jackson", title.position=c("right","top") )
...
set.seed( 1234 )
fit <- Mclust( dat3 )
jackson_dorling$cluster <- as.character( fit$classification )
summary( fit )
```
...
jax <- merge( jackson_dorling, d, by.x="GEOID", by.y="tractid2")


head( jax@data ) %>% pander()
```
...
```{r}
row.ids <- sapply( slot( jax, "polygons" ), function(x) slot( x, "ID" ) )
row.names( sea ) <- row.ids
```

```{r}
jax <- spTransform( jax, CRS("+proj=longlat +datum=WGS84") )
geojson_write( jax, file="jackson_dorling.geojson", geometry="polygon" )
```
